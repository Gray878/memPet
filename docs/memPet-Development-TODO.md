...
- [ ] 创建 ProactiveService 类 `[HGH]`：定时检查逻辑；上下文收集；推理触发条件
- [ ] 实现上下文收集 `[HGH]`：当前活动窗口；最近记忆检索；时间和日期；系统状态
- [ ] 实现推理逻辑 `[HGH]`：调用 LLM API；结果解析；错误处理
- [ ] 创建规则配置 `[HGH]`：工作时长提醒；休息建议；任务提醒；学习建议
- [ ] 实现规则匹配 `[HGH]`：条件判断；优先级排序；冷却时间
- [ ] 实现推理缓存 `[HGH]`：避免重复推理；结果缓存；缓存失效
- [ ] 创建 ProactiveMessage 组件 `[nil-byte]`：推理结果显示；建议操作按钮；忽略功能
