# memU-server 启动指南（含数据库）

memU-server 需要 PostgreSQL 数据库。以下是完整的启动步骤。
...
Write-Host "=== memU-server 启动脚本 ===" -ForegroundColor Green
...
# 4. 启动服务
Write-Host "[4/4] 启动 memU-server..." -ForegroundColor Yellow
Write-Host ""
uv run fastapi dev
```
