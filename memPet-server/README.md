# memPet-server: 桌面宠物专用版

memPet-server 是为 memPet 桌面宠物应用定制的后端服务，基于 memU 核心框架，提供记忆存储、检索和主动推理功能。
...
```bash
# 契约对齐测试
./.venv/bin/python -m pytest tests/test_contract_alignment.py -q
```

### 6. 导出 OpenAPI 契约

```bash
./.venv/bin/python scripts/export_openapi.py --output docs/openapi.json
```
...
- `POST /chat`：稳定通道（首发默认）
- `POST /chat/stream`：SSE 流式通道（可选增强）
...
兼容迁移期保留关键旧字段镜像（例如 `/chat` 的 `response` 顶层字段）。
...
- [测试指南](./docs/测试指南.md) - 如何测试 memPet-server

---

## 与 Electron 集成

memPet-server 作为 HTTP API 服务，需要在 Electron 主进程中调用：
...
// 存储对话记忆
async function memorizeConversation(messages) {
